...
void ChooseMenPanel::input(int m_x,int m_y,bool m_left,bool m_middle,bool m_right,bool click) {
    ...
    if( this->c_page == STATE_CHOOSEISLAND ) {
		if( m_left && click && this->button_nextisland != NULL && this->button_nextisland->mouseOver(m_x, m_y) ) {
            ...
            gamestate->closeConfirmWindow();
#ifdef WINRT
			char help_url[] = "http://www.greentwip.com/";
#endif
#if defined(WINRT)
			std::string url_string = help_url;
			std::wstring temp_string(url_string.begin(), url_string.end());

			auto display_url_string = ref new Platform::String(temp_string.c_str());

			Windows::Foundation::Uri^ uri = ref new Windows::Foundation::Uri(display_url_string);
